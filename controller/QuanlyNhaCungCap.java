...
public class QuanlyNhaCungCap extends NhaCungCap implements ControllerInterface{
	...
	public NhaCungCap[] getListNhaCungCap() throws FileNotFoundException{
    	String[] result = Stream.read("src/database/provider.txt");
	...
	public void Read() {
		...
		String header = String.format("| %-5s | %-25s | %-40s | %-11s | %-9s |", "Mã", "Tên NCC", "Địa chỉ", "Điện thoại" , "Fax");
		System.out.format("+-------+---------------------------+------------------------------------------+-------------+-----------+%n");
       ...
       for(int i = 0; i < ncc.length; i++) {
   		String read = String.format("| %-5s | %-25s | %-40s | %-11s | %-9s |", ncc[i].getMaNCC(), ncc[i].getTenNCC(), ncc[i].getDiaChi(), ncc[i].getSDT(), ncc[i].getFax());
	...
	public void Create() {
       ...
       if(check == 1) {
		...
		try {
			String inputStringData = getMaNCC() + ";" + getTenNCC() + ";" + getDiaChi() + ";" + getSDT() + ";" + getFax();
			Stream.addOneLine("src/database/provider.txt", inputStringData);
	...
	public void Update() {
		 ...
		 String row = String.format("%-5s %-25s %-4s %-11s %-11s", nccount.getMaNCC(), nccount.getTenNCC(), nccount.getDiaChi(), nccount.getSDT(), nccount.getFax());
		 ...
		 for(int i = 0; i < ncc.length; i++) {
			 ...
			 data[i] = ncc[i].getMaNCC() + ";" + ncc[i].getTenNCC() + ";" + ncc[i].getDiaChi() + ";" + ncc[i].getSDT() + ";" + ncc[i].getFax();
		 }
		 
		  try {
				Stream.addAll("src/database/provider.txt", data);
	...
	public NhaCungCap[] deleteNhaCungCap(NhaCungCap[] ncc, int index) {
	 ...
	 public void Search_byCategory() {
		 ...
		 for(int i = 0; i < ncc.length; i++) {
			 if(ncc[i].getTenNCC().toLowerCase().contains(key.toLowerCase()) || ncc[i].getMaNCC().toLowerCase().contains(key.toLowerCase())) {
	    ...
	    System.out.format("+-------+---------------------------+------------------------------------------+-------------+-----------+%n");
       for (int i = 0; i < result.length; i++) {
    	   String read = String.format("| %-5s | %-25s | %-40s | %-11s | %-9s |", result[i].getMaNCC(), result[i].getTenNCC(), result[i].getDiaChi(), result[i].getSDT(), result[i].getFax());
   		System.out.println(read);
       }
       System.out.format("+-------+---------------------------+------------------------------------------+-------------+-----------+%n");
		waitConsole();
