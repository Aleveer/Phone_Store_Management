...
public class QuanLyConfigurations extends Configuation implements DataAccessObject{
	public Configuation[] cfg;
	static Scanner input = new Scanner(System.in);
	public QuanLyConfigurations() throws FileNotFoundException {
		super();
		getListConfigurations();
	}
	public Configuation[] getListConfigurations() throws FileNotFoundException {
	        String[] result = Stream.read("src/database/configurations.txt");
	    ...
	    public void Read() throws FileNotFoundException {
	        ...
	        String header = String.format("| %-5s | %-10s | %-15s | %-30s | %-10s | %-10s | %-10s | %-10s | %-30s |", "ID", "Màn hình", "Hệ điều hành", "Cấu hình", "RAM", "Bộ nhớ", "Dung lượng pin", "Camera trước", "Camera Sau");
	        System.out.format("+-------+------------+-----------------+--------------------------------+------------+-----------------------------+--------------+--------------------------------+%n");
	        ...
	        for (Configuation configuation : cfg) {
	                String read = String.format("| %-5s | %-10s | %-15s | %-30s | %-10s | %-10s | %-14s | %-12s | %-30s |", configuation.getID_Product(), configuation.getScreen(), configuation.getOS(),
	    ...
	    public void Create() {
	        ...
	        try {
	            ...
	            Stream.addOneLine("src/database/configurations.txt", input);
	    ...
	    public void Update() {
	        try {
	            ...
	            for (int i = 0; i < cfg.length; i++) {
	                if (cfg[i].getID_Product().equals(ID_Product)) {
	                    input.nextLine();
	                    System.out.print("Nhập Screen: ");
	                ...
	                data[i] = cfg[i].getID_Product() + ";" + cfg[i].getScreen() + ";" + cfg[i].getOS()
							+ ";" + cfg[i].getChip() + ";" + cfg[i].getRam() + ";" + cfg[i].getMemory() + ";" + cfg[i].getBattery() + ";" + cfg[i].getFrontCamera() + ";" + cfg[i].getRearCamera();
	            }
	            try {
	                Stream.addAll("src/database/configurations.txt", data);
	    ...
	    public void Search_byCategory() {
	        ...
	        String header = String.format("| %-5s | %-10s | %-25s | %-30s | %-30s | %-10s | %-10s | %-20s | %-30s |", "ID", "Màn hình", "Hệ điều hành", "Cấu hình", "RAM", "Bộ nhớ", "Dung lượng pin", "Camera trước", "Camera Sau");
			System.out.format("+-------+------------+-----------------+--------------------------------+------------+-----------------------------+--------------+--------------------------------+%n");
	        System.out.println(header);

	        for (Configuation configuation : result) {
	            String read = String.format("| %-5s | %-10s | %-30s | %-30s | %-30s | %-10s | %-10s | %-10s | %-20s |", configuation.getID_Product(), configuation.getScreen(), configuation.getOS(),
	                    configuation.getChip(), configuation.getRam(), configuation.getMemory(), configuation.getBattery(), configuation.getFrontCamera(), configuation.getRearCamera());
	            System.out.println(read);
	        }
            waitConsole();
