...
public class QuanLyNhanVien extends NhanVien implements DataAccessObject {
    ...
    public NhanVien[] getListNhanVien() throws FileNotFoundException {
        String[] result = Stream.read("src/DuLieu/employee.txt");
        nv = new NhanVien[result.length];
        for (int i = 0; i < result.length; i++) {
            String[] row = result[i].split(";");
            nv[i] = new NhanVien(row[0], row[1], row[2], row[3], Integer.parseInt(row[4]), row[5], row[6], row[7], row[8]);
        }
        return nv;
    }
    ...
    public void read() throws FileNotFoundException {
        ...
        for (NhanVien nhanVien : nv) {
            if (nv[0].getID_Worker().contains("nv")) {
    ...
    public void Add() {
        try {
            ...
            try {
                ...
                Stream.addOneLine("src/DuLieu/employee.txt", input);
    ...
    public void Update() {
        try {
            ...
            for (int i=0; i < nv.length; i++) {
                if (ID_NhanVien.equalsIgnoreCase(getID_Worker())) {
                    System.out.println("Nhập thông tin nhân viên:");
                    super.Add();
            ...
            try {
                Stream.addAll("src/DuLieu/employee.txt", data);
